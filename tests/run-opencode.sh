...
rm -f "$attempt_file"
export OPENCODE_RETRY_ON_REGEX=""
export OPENCODE_RETRY_PROFILE="github-network"
profile_output="$("$repo_root/run-opencode/run-opencode.sh" 2>&1)"

if [[ "$profile_output" != *"success on attempt 3"* ]]; then
  printf 'expected retry profile output, got:\n%s\n' "$profile_output" >&2
  exit 1
fi

profile_attempts="$(<"$attempt_file")"
if [[ "$profile_attempts" != "3" ]]; then
  printf 'expected retry profile to use 3 attempts, got %s\n' "$profile_attempts" >&2
  exit 1
fi
